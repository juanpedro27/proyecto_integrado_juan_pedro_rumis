...
foreach ($_GET as $key => $Cliente_id)
    
    // SELECCIONAMOS EL PEDIDO POR LA ID DEL CLIENTE QUE LO REALIZA

    if ($result1 = $connection->query("SELECT * FROM pedidos where Clientes_id=$Cliente_id;")) {

        // BORRADO DEL PEDIDO

        if ($result6 = $connection->query("DELETE FROM pedidos where Clientes_id=$Cliente_id;")) {  

            echo "<center><h3>El pedido ha sido borrado.</h3></center>";
